...
const createSendToken = (user, statusCode, res) => {
    ...
    const cookieOptions = {
        ...
        // secure: true, // this is only for production environment, so this is omitted here
        httpOnly: true
    };
...
exports.login = catchAsync(async (req, res, next) => {
    ...
    // if user null then correctPassword property cannot read so below code used in if condition to avoid waird error message.
...
exports.checkAuthorization = catchAsync(async (req, res, next) => {
    ...
    if (!token) {
        return next(new AppError('You are not logged in. You must log in to get acces.', 401));
    }
...
exports.isLoggedIn = async (req, res, next) => {
    ...
    if (req.cookies.jwt) {
        try {
            ...
            const verifiedToken = await promisify(jwt.verify)(req.cookies.jwt, process.env.JWT_SECRET);
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    // 1) Get the users e-mail and check if user's e-mail exists.
    const user = await User.findOne({
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    // 2) Is token has not expired and there is user, then set the new password
    if (!user) {
...
exports.updateMyPassword = catchAsync(async (req, res, next) => {
    ...
    if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
        return next(new AppError('Your password is not correct! Please enter a correct password!', 401));
    }
